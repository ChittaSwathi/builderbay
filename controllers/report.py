...
class FinancialReportController(FinancialReportController):
    ...
    def get_report(self, model, options, output_format, token, financial_id=None, **kw):
        ...
        _logger.error("%s report ", report_obj)
        ...
        _logger.error("%s report ", report_name)
        try:
            if output_format == 'xlsx':
                ...
                context_ledger_type = request.env.context.get('ledger_type',False)
                if ledger_type == 'bs' or context_ledger_type == 'bs' and report_name == 'partner_ledger':
                    response.stream.write(report_obj.bs_partner_ledger_get_xlsx(options))
                else:
